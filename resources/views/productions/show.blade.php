        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {{-- <div class="bg-gray-50 rounded-lg p-4">
                ...
                <form id="addLaborForm" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label for="workers_count" class="block text-sm font-medium text-gray-700 mb-1">Nombre d'ouvriers <span class="text-red-500">*</span></label>
                            <input type="number" id="workers_count" name="workers_count" min="1" step="1" class="form-input-focus w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200" required>
                            <div id="workers_count_error" class="text-red-500 text-xs mt-1 hidden"></div>
                        </div>
                        <div>
                            <label for="worker_price" class="block text-sm font-medium text-gray-700 mb-1">Prix par ouvrier <span class="text-red-500">*</span></label>
                            <input type="number" id="worker_price" name="worker_price" min="0.01" step="0.01" class="form-input-focus w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200" required>
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                ...
                <form id="addLaborForm" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label for="additional_labor" class="block text-sm font-medium text-gray-700 mb-1">Main d'œuvre supplémentaire</label>
                            <textarea id="additional_labor" name="additional_labor" rows="2" class="form-input-focus w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200" placeholder="Précisez ici tout travail supplémentaire effectué (transport, expertise spécifique, etc.)"></textarea>
                            <div class="text-xs text-gray-500 mt-1">Description du travail supplémentaire (facultatif)</div>
                            <div id="additional_labor_error" class="text-red-500 text-xs mt-1 hidden"></div>
                        </div>
                        <div>
                            <label for="additional_cost" class="block text-sm font-medium text-gray-700 mb-1">Montant supplémentaire</label>
                            <input type="number" id="additional_cost" name="additional_cost" min="0" step="0.01" class="form-input-focus w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200" placeholder="0.00">
                            <div class="text-xs text-gray-500 mt-1">Coût supplémentaire (facultatif)</div>
                            <div id="additional_cost_error" class="text-red-500 text-xs mt-1 hidden"></div>
                        </div>
                    </div>
            ...
            <!-- Liste de la main d'œuvre ajoutée -->
<div>
            ...
            <table class="min-w-full divide-y divide-gray-200" id="laborTable">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Suppl. (Montant)</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="laborTableBody">
                    ...
                    <tr data-id="{{ $labor->id }}">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{{ $labor->additional_cost ? number_format($labor->additional_cost, 2, ',', ' ') : '-' }}</div>
                        </td>
                        <td class="px-4 py-3">
                            <div class="text-sm text-gray-900 line-clamp-2" title="{{ $labor->additional_labor }}">
                                {{ $labor->additional_labor ?: '-' }}
                            </div>
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">
                                {{ number_format(($labor->workers_count * $labor->worker_price) + ($labor->additional_cost ?? 0), 2, ',', ' ') }}
                            </div>
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap">
                            <div class="text-sm font-bold text-gray-900" id="laborTotal">
                                {{ number_format($laborEntries->sum(function($l) { 
                                    return ($l->workers_count * $l->worker_price) + ($l->additional_cost ?? 0); 
                                }), 2, ',', ' ') }}
                            </div>
                        </td>
                        <td></td>
    ...
    <div class="px-4 py-3 bg-white border-t border-gray-200">
        ...
        <!-- Affichage de la main d'œuvre pour une production terminée -->
<div class="bg-white border border-gray-200 rounded-lg overflow-hidden">
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
