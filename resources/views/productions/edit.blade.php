...
@section('page-title', 'Modifier la production')
@section('page-subtitle', 'Modifier les informations de la production')

@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-lg shadow-sm p-6">
            <form action="{{ route('production.update', $production->id) }}" method="POST" id="productionForm">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="col-span-2">
                        ...
                        <input type="text" name="production_name" id="production_name" value="{{ $production->production_name }}" class="form-input-focus w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200" required>
                    ...
                    <div>
                        ...
                        <select name="type_id" id="type_id" class="form-select-focus w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200" required>
                            ...
                            @foreach($types as $type)
                                <option value="{{ $type->id }}" {{ $production->type_id == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                            @endforeach
                        </select>
                        @error('type_id')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="date" name="production_date" id="production_date" value="{{ $production->production_date }}" class="form-input-focus w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="qte_production" id="qte_production" value="{{ $production->qte_production }}" step="0.01" min="0" class="form-input-focus w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200" required>
                    ...
                    <input type="hidden" name="status" value="{{ $production->status }}">
                    ...
                    <div>
                        ...
                        <div class="px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg">
                            ...
                            @endif
                            <p class="text-xs text-gray-500 mt-1">Le statut n'est pas modifiable depuis cette interface.</p>
                        </div>
                    </div>
